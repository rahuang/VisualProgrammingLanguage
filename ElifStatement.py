...
class ElifStatement(IfStatement):
    def __init__(self, x, y, condition="", do=[], width=100, height=25):
        ...
        self.selected = False
        v1 = StringVar()
        self.conditionInput = Entry(width=27,textvariable=v1)
    ...
    def draw(self, canvas):
        ...
        if(self.selected):
            outlineColor = "red"
        canvas.create_rectangle(self.x-self.width,self.y-self.height, self.x+self.width, self.y+self.height, fill="CadetBlue2", outline=outlineColor)
        canvas.create_text(self.x-self.width+5, self.y-self.height+5, text="elif: ", anchor="nw", font="Arial 10 bold")
        canvas.create_window(self.x-self.width+30, self.y-self.height+1, window=self.conditionInput, anchor="nw")
